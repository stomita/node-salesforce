...
Connection.prototype.loginBySoap = function(username, password, callback) {
  ...
  request({
    ...
    var serverUrl = m && m[1];
    var m = response.body.match(/<metadataServerUrl>([^<]+)<\/metadataServerUrl>/);
    ...
    sUrl = path.normalize(split.slice(0, 3).join('/')),
    sPath = path.normalize(split.slice(3, split.length).join('/'));
